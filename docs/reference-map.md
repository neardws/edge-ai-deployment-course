...
下面这些图解是本课程已经吸收进章节的代表性素材。阅读资料地图时，先看图解对应哪类问题，再去读对应来源，不要从链接列表里随机跳。课程正文保留来源链接，并重画为自己的流程图或表格。

```mermaid
flowchart LR
  A["输入流水线"] --> B["前置知识"]
  C["量化方案"] --> D["PTQ / LLM 量化"]
  E["benchmark lab"] --> F["Profiling / 报告"]
  G["Jetson 设备"] --> H["Jetson 实作"]
  I["local-first agent"] --> J["VLM / Agent"]
```

| 原图 | 先读哪类资料 | 回到课程哪里 |
| --- | --- | --- |
| [full NLP pipeline](https://huggingface.co/datasets/huggingface-course/documentation-images/resolve/main/en/chapter2/full_nlp_pipeline.svg) | Hugging Face LLM Course / Transformers | 前置知识、baseline、local API |
| [quantization schemes](https://raw.githubusercontent.com/vllm-project/vllm-project.github.io/main/assets/figures/2026-06-03-deeplearning-ai-course/quantization-schemes.png) | DeepLearning.AI / PyTorch / ONNX 量化 | 量化数学、PTQ/QAT、LLM 量化 |
| [benchmarking lab](https://raw.githubusercontent.com/vllm-project/vllm-project.github.io/main/assets/figures/2026-06-03-deeplearning-ai-course/benchmarking-lab.png) | vLLM / MLPerf / llama-bench | 推理加速、样例日志、最终报告 |
| [Jetson 设备族](https://www.jetson-ai-lab.com/images/hero/jetson-family-line_50pcnt.png) | Jetson docs / Jetson AI Lab | 环境矩阵、Jetson 实验 |
| [local-first agent](https://raw.githubusercontent.com/microsoft/edgeai-for-beginners/main/WorkshopForAgentic/imgs/arch.png) | Microsoft EdgeAI / Agent 资料 | VLM/Agent、系统复盘 |
...
| 图解 | 哪些结构要重画成本课程图表，来源链接保留在哪里 | 不贴来源或许可不清的第三方原图 |
...
- 先从 [可吸收原始资料暂存](/docs/raw-reference-intake) 取图和要点：正文优先重画图示，原图保留链接，许可不清的只放链接。
...
- 不复制资料正文或课程作业。
- 配图按许可处理：正文优先重画为 Mermaid、表格或原创示意图；许可不清的图只放链接。
